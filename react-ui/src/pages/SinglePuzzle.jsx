...
export default function SinglePuzzle({ puzzleId }) {
  ...
  useEffect(() => {
    fetch(`${window.location.protocol}//${window.location.host}/data/puzzle`)
      .then(data => data.json())
      .then(jsonData => setPuzzleData(JSON.parse(jsonData)))
      .catch(console.log);
  }, []);
    ...
    <>
      ...
      {puzzleData[id] ?
        <>
          ...
          <Buttons callback={changeSelected} puzzle={puzzleData[id]} status={selected}/>
          <div className="file">
